...
var Fever_damage = false

func _on_body_entered(body):
	for i in attackable_list:
		if body.is_in_group(i):
			if get_tree().root.get_node("Lobby_Menu/Main").monster_killed <= 5:
				if get_parent().get_parent().get_node("AnimationPlayer").current_animation == "Attack":
					...
					body.health -= attack_value
				attacked.emit()
			elif get_tree().root.get_node("Lobby_Menu/Main").monster_killed > 5:
				Fever_damage = true
				get_parent().get_parent().fever = true
				if get_parent().get_parent().get_node("AnimationPlayer").current_animation == "Attack":
					attack_value = 50
					body.get_node("Area2D").damage_dealt = attack_value
					body.get_node("Area2D").knock_back.emit()
